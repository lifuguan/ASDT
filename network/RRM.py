...
class Net(network.resnet38d.Net):
    def __init__(self):
        ...
        # refine
        self.fc8_seg_conv3 = nn.Conv2d(4096, 512, (3, 3), stride=1, padding=12, dilation=12, bias=True)
        torch.nn.init.xavier_uniform_(self.fc8_seg_conv3.weight)

        self.fc8_seg_conv4 = nn.Conv2d(512, 21, (3, 3), stride=1, padding=12, dilation=12, bias=True)
    ...
    def forward(self, x, require_seg = True, require_mcam = True):
        ...
        if require_seg == True and require_mcam == True:
            ...
            x_seg2 = self.fc8_seg_conv3(x_seg2)
            x_seg2 = F.relu(x_seg2)
            x_seg2 = self.fc8_seg_conv4(x_seg2)

            return x, cam, x_seg, x_seg2
    ...
    def forward(self, x, require_seg=True, require_mcam= True):
        if require_seg == True and require_mcam == True:
            ...
            with torch.enable_grad():
                xf1, cam1, seg1, seg2 = super().forward(x,require_seg=True, require_mcam=True)
            with torch.no_grad():
            ...
            seg_list.append(seg1)  # for original scale
            seg2_list.append(seg2)

            return xf_temp, cam, seg_list, seg2_list
        ...
        if require_mcam == False and require_seg == True:
            xf, cam, seg, seg2 = super().forward(x, require_seg=True, require_mcam=True)
            return seg, seg2
